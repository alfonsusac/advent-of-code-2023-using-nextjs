...
const puzzleInput = `...................15....904...........850.................329...................13....................................871....816....697....
...
export const sampleInput = `467..114..
...*......
..35..633.
......#...
617*......
...
export default async function Day3Part1() {
  const input = puzzleInput

  const rows = input.split('\n')
  const size_y = rows.length
  const size_x = rows[0].length

  function isOutOfBounds(x: number, y: number) {
    return x < 0 || y < 0 || x > size_x || y > size_y
  }

  console.log(`Y: ${size_y} | X: ${size_x}`)

  let sum = 0

  let symbolMap: { [key: number]: { [key: number]: string } } = {}
  let numberPartArr: NumberPart[] = []
  let markerMap: { [key: number]: { [key: number]: NumberPart[] } } = {}
  const gearList: Gear[] = []

  let ycurr = 0
  for (const row of rows) {
    console.log(ycurr + ' ' + row)
    let digit = ''
    let xcurr = 0
    for (const char of row) {
      if (char.match(/\d/)) {
        ...
        if (digit.length > 0) {
          console.log("Found potential np: " + digit)
          numberPartArr.push(new NumberPart(xcurr - digit.length, ycurr, digit, markerMap))
        }
        if (char === '*') {
          console.log("Found Gear: " + `${xcurr} ${ycurr}`)
          gearList.push(new Gear(xcurr, ycurr))
        }
        digit = ''
      }
      if (xcurr === size_x - 1 && digit.length > 0) {
        console.log("Found potential np: " + digit)
        numberPartArr.push(new NumberPart(xcurr - digit.length, ycurr, digit, markerMap))
      }
      xcurr++
  ...
  const correctNumberPart: number[] = []

  for (const gear of gearList) {
    const area = gear.area

    let adjacentNP = new Map<number, true>()
    for (let y = area.p1.y; y <= area.p2.y; y++) {
      for (let x = area.p1.x; x <= area.p2.x; x++) {
        if (isOutOfBounds(x, y)) continue
        if (markerMap?.[y]?.[x]) {
          markerMap[y][x].forEach(m => {
            console.log(`${y} ${x} => ${markerMap[y][x]}`)
            adjacentNP.set(m.number, true)
          })
        }
      }
    }
    console.log(`gear ${gear.x} ${gear.y}: ` + JSON.stringify(adjacentNP.size))
    if (adjacentNP.size === 2) {
      const keys = adjacentNP.keys()
      let p1 = keys.next().value;
      let p2 = keys.next().value;
      sum += p1 * p2
    }
  }
  console.log(JSON.stringify(correctNumberPart))
...
class NumberPart {
  ...
  readonly adjacentNumberArea: Area
  readonly number: number

  constructor(readonly x: number, readonly y: number, readonly numberStr: string, markerMap: { [key: number]: { [key: number]: NumberPart[] } }) {
    this.number = parseInt(numberStr)
    const digitCount = numberStr.length
    this.adjacentNumberArea = new Area(new Point(x, y), new Point(x + digitCount - 1, y))
    this.adjacentNumberArea.apply(({ x, y }) => {
      if (!markerMap[y]) markerMap[y] = {}
      if (!markerMap[y][x]) markerMap[y][x] = []
      markerMap[y][x].push(this)
    })
  }

  toJSON() {
    return `${this.number} at ${this.x} ${this.y}`
  }
  toString() {
    return `${this.number} at ${this.x} ${this.y}`
  }
...
class Gear {
  ...
  constructor(readonly x: number, readonly y: number) {
    this.area = new Area(new Point(x - 1, y - 1), new Point(x + 1, y + 1))
  }
}
