...
export default async function Day3Part1() {
    ...
    for (const char of row) {
      if (char.match(/\d/)) {
        ...
        if (digit.length > 0) {
          ...
          numberPartArr.push(new NumberPart(xcurr - digit.length, ycurr, digit))
        } else {

        }
        if (char !== '.') {
          console.log("Found Symbol: " + `${xcurr} ${ycurr}`)
          if(!symbolMap[xcurr]) symbolMap[xcurr] = {}
          symbolMap[xcurr][ycurr] = char
        }
        digit = ''
      }
  ...
  // console.log(numberPartArr)
  console.log(symbolMap)

  const correctNumberPart: number[] = []

  for (const numberPart of numberPartArr) {
    const area = numberPart.adjacentSymbolArea
    ...
    for (let y = area.y1; y <= area.y2; y++) {
      for (let x = area.x1; x <=area.x2; x++) {
        ...
        if (symbolMap?.[x]?.[y]) {
          correctNumberPart.push(parseInt(numberPart.number))
          // console.log("Found number part with adjacent symbol! " + numberPart.number)
          sum += parseInt(numberPart.number)
          found = true
          break;
        }
      }
...
class NumberPart {
  ...
  constructor(
    ...
    this.adjacentSymbolArea = {
      x1: xStart - 1,
      y1: yStart - 1,
      x2: xStart + digitCount,
      y2: yStart + 1,
    }
  }
