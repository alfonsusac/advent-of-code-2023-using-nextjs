...
export default function Page() {
  ...
  function getDistanceAndNextPipe(x: number, y: number, value: number) {

    if (distMap[y][x]) return

    distMap[y][x] = { distance: value + 1 }
    const thisChar = input[y][x]
    if (thisChar === '|') {
      queue.push(new Pos(x, y + 1))
      queue.push(new Pos(x, y - 1))
    }
    if (thisChar === '-') {
      queue.push(new Pos(x + 1, y))
      queue.push(new Pos(x - 1, y))
    }
    if (thisChar === 'F') {
      queue.push(new Pos(x + 1, y))
  ...
  if (['|', '7', 'F'].includes(input[S.y - 1][S.x])) {
    getDistanceAndNextPipe(S.y - 1, S.x, 0)
  }
  if (['-', '7', 'J'].includes(input[S.y][S.x + 1])) {
    getDistanceAndNextPipe(S.y, S.x + 1, 0)
  }
  if (['-', 'L', 'F'].includes(input[S.y][S.x - 1])) {
    getDistanceAndNextPipe(S.y, S.x - 1, 0)
  }
  if (['|', 'L', 'J'].includes(input[S.y + 1][S.x])) {
    getDistanceAndNextPipe(S.y + 1, S.x, 0)
  }
  ...
  return <pre style={ { lineHeight: "0.7", color: "grey" } }>{ input.split('\n').map((row, y) => {
    return <>{ row.split("").map((char, x) => {
      return char === "-" ? "─" : char === '7' ? '┐' : char === "L" ? '└' : char === "J" ? "┘" : char === "|" ? "│" : char === "." ? <span style={ { opacity: '0.1' } }>.</span> : char === "S" ? <span style={ { color: 'gold' } }>S</span> : char === "F" ? "┌" : char
    }) }<br /></>
