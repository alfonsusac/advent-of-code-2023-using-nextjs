...
export default function Page() {
  ...
  let distMap = new2DArr<undefined | distance>(input.length, input[0].length, undefined)
  // find S
  ...
  function getDistanceAndNextPipe(y: number, x: number, value: number) {
    ...
    // console.log(`Getting Distance: ${x} ${y} ${value} ${rows[y][x]} | queue: ${queue.map(e => `${e.pos.y} ${e.pos.x}`)}`)

    if (value > maxDist) {
      maxDist = value
      maxDistCord = new Pos(x, y)
    }

    distMap[y][x] = { distance: value }
    const thisChar = rows[y][x]

    // console.log(thisChar)

    if (thisChar === '|') {
      queue.push({ pos: new Pos(x, y + 1), val: value + 1 })
      queue.push({ pos: new Pos(x, y - 1), val: value + 1 })
    }
    if (thisChar === '-') {
      queue.push({ pos: new Pos(x + 1, y), val: value + 1 })
      queue.push({ pos: new Pos(x - 1, y), val: value + 1 })
    }
    if (thisChar === 'F') {
      queue.push({ pos: new Pos(x + 1, y), val: value + 1 })
